...
SCHEMA_VERSION = "HYPERMATH-PUBLIC-RELEASE-1.0"
ARCHIVE_STEM = "hypermath-foundations"
DISTRIBUTION_NAME = "hypermath-foundations"
IMPORT_PACKAGE = "hypermath_foundations"
CONSOLE_SCRIPTS = {
...
def _verify_python_distributions(wheel: Path, sdist: Path, release: str) -> None:
    ...
    if scripts != CONSOLE_SCRIPTS:
        raise ReleaseError(f"wheel console scripts differ from the declared set: {scripts}")
...
def build_source(repo: Path, ref: str, release: str, output_dir: Path) -> tuple[Path, Path]:
    ...
    with tempfile.TemporaryDirectory(prefix="hypermath-source-archive-") as temporary:
    ...
    manifest = {
        ...
        "release": release,
        "scope": (
            "public Hypermath formal foundations, specifications, proofs, "
            "lean4 models, examples, tests, and public project process"
        ),
        "source": {
...
def _build_wheel_once(source_archive: Path, prefix: str, destination: Path, epoch: str) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="hypermath-wheel-") as temporary:
...
def _build_sdist_once(
    ...
    destination.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix="hypermath-sdist-") as temporary:
        temporary_path = Path(temporary)
        with zipfile.ZipFile(source_archive) as bundle:
...
def build_all(
    ...
    with tempfile.TemporaryDirectory(prefix="hypermath-wheel-builds-") as temporary:
        temporary_path = Path(temporary)
    ...
    with tempfile.TemporaryDirectory(prefix="hypermath-sdist-builds-") as temporary:
        temporary_path = Path(temporary)
...
compare_directories = compare_artifact_directories
